import 'dart:math';

import '../../../../../core/data/repos/services/base_shared_preference.dart';
import '../../models/base_calculator_model.dart';

class PropertyLoanCtrl {
  static const String _key = 'property_loan_result';

  /// Calculate EMI, total interest, and total amount payable for property loan.
  /// [amount] = Principal loan amount
  /// [rate] = Annual interest rate in %
  /// [time] = Loan tenure amount
  /// [timeType] = 'Yearly' or 'Monthly'
  static BaseCalculatorModel calculate({
    required double amount,
    required double rate,
    required double time,
    required String timeType,
  }) {
    final P = amount;
    final annualRate = rate;

    // Convert tenure to years if given in months
    final years = timeType.toLowerCase() == 'monthly' ? time / 12 : time;

    final monthlyRate = annualRate / 12 / 100;
    final totalMonths = (years * 12).round();

    // EMI formula
    final emi = (P * monthlyRate * pow(1 + monthlyRate, totalMonths)) /
        (pow(1 + monthlyRate, totalMonths) - 1);

    final totalPayable = emi * totalMonths;
    final interest = totalPayable - P;

    return BaseCalculatorModel(
      amount: P,
      result1: emi,
      // EMI
      result2: interest,
      // Interest amount
      result3: totalPayable,
      // Total amount payable
      rate: annualRate,
      time: years,
    );
  }

  static Future<void> save(BaseCalculatorModel model) async {
    await BaseSharedPreference.save(_key, model);
  }

  static Future<BaseCalculatorModel?> load() async {
    return await BaseSharedPreference.load(_key);
  }

  static Future<void> clear() async {
    await BaseSharedPreference.clear(_key);
  }
}
